...
public class SigarUtils {
	...
	public static List<SigarInfoEntity> getCpuInfos() throws SigarException {
		...
		// cpuInfoList.add(new SigarInfoEntity(String.valueOf(cpuInfo.getMhz()),
		// "CPU的总量MHz" + i));
		...
		// cpuInfoList.add(new SigarInfoEntity(String.valueOf(cpuInfo.getCacheSize()),
		// "缓冲存储器数量" + i));
		// }
		...
		for (int i = 0; i < cpuPercs.length; i++) {
			...
			// cpuInfoList.add(new SigarInfoEntity(String.valueOf(cpuPerc.getUser()),
			// "CPU用户使用率" + i));
			// cpuInfoList.add(new SigarInfoEntity(String.valueOf(cpuPerc.getSys()),
			// "CPU系统使用率" + i));
			// cpuInfoList.add(new SigarInfoEntity(String.valueOf(cpuPerc.getWait()),
			// "CPU当前等待率" + i));
			// cpuInfoList.add(new SigarInfoEntity(String.valueOf(cpuPerc.getNice()),
			// "CPU当前错误率" + i));
			// cpuInfoList.add(new SigarInfoEntity(String.valueOf(cpuPerc.getIdle()),
			// "CPU当前空闲率" + i));
			// cpuInfoList.add(new SigarInfoEntity(String.valueOf(cpuPerc.getCombined()),
			// "CPU总的使用率" + i));
			cpuInfoList.add(
					new SigarInfoEntity(dft.format(cpuPerc.getCombined()),
							String.format("第 %s 个CPU总的使用率", i)));
			
			totalCombined+=cpuPerc.getCombined();

		}
		
		cpuInfoList.add(
				new SigarInfoEntity(dft.format(totalCombined * 1.0 / 4 * 100) + "%",
						"CPU平均使用率"));
	...
	public static List<SigarInfoEntity> getMemoryInfos() throws SigarException {
		...
		memoryInfoList.add(new SigarInfoEntity(dft.format(mem.getTotal() * 1.0 / divisor) + "G", "内存总量"));
		memoryInfoList.add(new SigarInfoEntity(dft.format(mem.getUsed() * 1.0 / divisor) + "G", "当前内存使用量"));
		memoryInfoList.add(new SigarInfoEntity(dft.format(mem.getFree() * 1.0 / divisor) + "G", "当前内存剩余量"));

		memoryInfoList
				.add(new SigarInfoEntity(dft.format(mem.getUsed() * 1.0 / mem.getTotal() * 100) + "%", "内存使用百分比"));

		Swap swap = getInstance().getSwap();
		memoryInfoList.add(new SigarInfoEntity(dft.format(swap.getTotal() / divisor) + "G", "交换区总量"));
		memoryInfoList.add(new SigarInfoEntity(dft.format(swap.getUsed() / divisor) + "G", "当前交换区使用量"));
		memoryInfoList.add(new SigarInfoEntity(dft.format(swap.getFree() / divisor), "当前交换区剩余量"));

		memoryInfoList
				.add(new SigarInfoEntity(dft.format(swap.getUsed() * 1.0 / swap.getTotal() * 100) + "%", "交换区使用百分比"));
	...
	public static List<ProcessInfo> getProcessInfo() {
		...
		try {
			...
			for (long pid : pids) {
				...
				if (info.getName().contains("cmd.exe") || info.getName().contains("java")) {
